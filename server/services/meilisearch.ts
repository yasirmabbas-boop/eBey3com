...
function listingToDocument(listing: Listing): Record<string, unknown> {
  const l = listing as Listing & Record<string, unknown>;
  return {
    id: l.id,
    title: l.title ?? "",
    ...
    auctionEndTime:
      l.auctionEndTime instanceof Date ? l.auctionEndTime.toISOString() : l.auctionEndTime,
    shippingCost: l.shippingCost ?? 0,
    shippingType: (l.shippingType as string) ?? "seller_pays",
  };
}
